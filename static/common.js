...
function initializeLanguage() {
    ...
    let savedLang = localStorage.getItem('ipfsbed_language');
    if (!savedLang) {
        const browserLang = navigator.language || navigator.userLanguage;
        savedLang = browserLang.startsWith('zh') ? 'zh-CN' : 'en';
    }
...
function changeLanguage(lang) {
    ...
    localStorage.setItem('ipfsbed_language', lang);
    
    // Update page language
    if (typeof updatePageLanguage === 'function') {
        updatePageLanguage();
    } else if (typeof updateSharePageLanguage === 'function') {
        updateSharePageLanguage();
    } else if (typeof updateBatchSharePageLanguage === 'function') {
        updateBatchSharePageLanguage();
    } else if (typeof updateHistoryPageLanguage === 'function') {
...
function _t(key, params = {}) {
    ...
    let translation = window.translations[window.currentLang][key];
    if (!translation) {
        // Fallback to English
        translation = window.translations['en'] && window.translations['en'][key];
        if (!translation) {
            return key;
        }
    }
...
// Update page language function (for index.html)
